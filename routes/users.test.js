...
describe("GET /users/:username", function () {
  ...
  test("works for matching username", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.body).toEqual({
  ...
  test("unauth for NOT-matching username", async function () {
    const resp = await request(app)
      .get(`/users/u1`)
...
describe("PATCH /users/:username", () => {
  ...
  test("works for matching username: set new password", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        password: "new-password",
  ...
  test("unauth for NOT-matching username: set new password", async function () {
    const resp = await request(app)
      .patch(`/users/u1`)
      .send({
        password: "new-password",
...
describe("DELETE /users/:username", function () {
  ...
  test("not found if user missing as user", async function () {
    const resp = await request(app)
      .delete(`/users/nope`)
      .set("authorization", `Bearer ${u1Token}`);
    expect(resp.statusCode).toEqual(401);
